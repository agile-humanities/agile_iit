...
//console.log(imageSizePrefix);

function calculateImagePrefix(number){
	var result;
	if (number>=800) { result='800px';}
	else if (number>=700) {result='600px';}
	else if (number>=600) {result='600px';}
	else if (number>=500) {result='400px';}
	else if (number>=400) {result='400px';}
	else if (number>=300) {result='200px';}
	else if (number>=200) {result='200px';}
	else {result='200px';}
	return result;
}
...
function replaceImagePrefix(prev,next){
	...
	//console.log("changed in replace" + previousImagePrefix);
}
   ...
   var that=this;
	...
	.fail(function(){ 
		...
		//console.log("data lrg url changed");
			...
			.fail(function(){ 
				//console.log("no in tmpimages folder");
				//console.log("fail");
 	...
 	if (tmp1==="" || tmp2==="")
		...
		$.post("cropviews.php", values, function(data){
			...
			$("#overlay2").append(data);
                     $("#cropform2").onSubmit=function(){ return checkCoords();}
...
$(document).on('submit', '#cropform2', function(e){
		...
		if (parseInt($('#w').val())) {
			...
			$.post("crop2.php", values, function(data){
				// alert(data);
				 $("#results").append(data);
				// $(".draggable").draggable({containment: "window"}).resizable();
...
$(document).on('click', '#results', function(event){
		...
		if ($target.is(".ui-icon-close")){
			...
			$target.parent().parent().fadeOut(300, function() { $(this).remove(); })
		}
			...
			if (x>=image2_offset.left && x<=image2_offset.left+image2_width && 
				...
				$.post("crop3.php", values, function(data){
					//alert(data);
					var myWindow=window.open('', 'cmpWindow', 'width=800, height=400, scrollbars=yes');
